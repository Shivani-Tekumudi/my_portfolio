  ...
  return (
    <section className="min-h-screen px-6 xl:px-20   py-10 bg-main text-xl">
      <Link href="/#projects" scroll={true}>
        <span className="text-primary block pb-5 text-sm">
          <CircleArrowLeft className="text-primary inline" /> Back</span>
      </Link>
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
        ...
        <article className="md:col-span-3">
          ...
          {project.projectDetails.problemStatement && (
            <div id="overview" className=" p-6 bg-transparentder border  border-accent rounded-lg shadow-lg p-6 md:p-8 shadow-sm  rounded-2xl mb-5">
              <h2 className="text-2xl font-semibold mb-3 text-primary4">Problem Statement</h2>
          ...
          {project.projectDetails.overview && (
            <div id="overview" className=" p-6 bg-transparentder border  border-accent rounded-lg shadow-lg p-6 md:p-8 shadow-sm  rounded-2xl mb-5">
              <h2 className="text-2xl font-semibold mb-3 text-primary4">Overview</h2>
              <p className="text-grey leading-relaxed text-lg">
                {project.projectDetails.overview}
              </p>
              <Image alt="skill"
          ...
          {project.projectDetails.researchAndPlanning && (
            <div id="research" className=" p-6  md:p-8 sbg-transparentder border  border-accent rounded-lg shadow-lg mb-6">
              <h2 className="text-2xl font-semibold mb-3 text-primary4">Research</h2>
          ...
          {project.projectDetails.architectureAndSetup && (
            <div id="architecture" className=" p-6  md:p-8 sbg-transparentder border  border-accent rounded-lg shadow-lg mb-6">
              <h2 className="text-2xl font-semibold mb-3 text-primary4">{isDesignProject ? 'Design & Prototype' : 'Architecture & Setup'}</h2>
              <div className="text-grey leading-relaxed text-lg">
                <ReactMarkdown  >
                  {project.projectDetails.architectureAndSetup}
                </ReactMarkdown ></div>

              <h4 className="text-2xl font-semibold mb-3 text-primary4 pt-5 pb-3"> Tools</h4>
              {project.tools.map((ele, id) => {
                return (
                  <span className="rounded-md px-3 py-2 mr-4 text-base font-normal transition-all duration-200 border  border-accent  bg-card-bg3" key={id}> {ele}</span>)

              })}

              {isDesignProject ?
                <><h4 className="text-2xl font-semibold mb-3 text-primary4 pt-6"> User Flow</h4><div className="text-primary text-lg bg-surface p-5"><ReactMarkdown>{project.projectDetails.systemFlow}</ReactMarkdown></div></> : ''}
              <h4 className="text-2xl font-semibold mb-3 text-primary4 pt-6" > System Flow</h4>

              <div className="text-primary text-lg bg-surface p-5">
                <ReactMarkdown  >{project.projectDetails.systemFlow}</ReactMarkdown></div>
          ...
          {project.projectDetails.coreFeaturesOrKeySolutions && (
            <div id="core" className=" p-6  md:p-8 sbg-transparentder border  border-accent rounded-lg shadow-lg mb-6">
              <h2 className="text-2xl font-semibold mb-3 text-primary4">Core Features</h2>
              <div className="text-grey leading-relaxed text-lg">
                <ReactMarkdown  >
          ...
          {project.projectDetails.challengesAndLearnings && (
            <div id="challenges" className=" p-6  md:p-8 sbg-transparentder border  border-accent rounded-lg shadow-lg mb-6">
              <h2 className="text-2xl font-semibold mb-3 text-primary4">Challenges And Learnings</h2>
              <div className="text-grey leading-relaxed text-lg">
                <ReactMarkdown  >
                  {project.projectDetails.challengesAndLearnings}
                </ReactMarkdown ></div>
          ...
          {project.projectDetails.testingAndFeedback && (
            <div id="testing" className=" p-6  md:p-8 sbg-transparentder border  border-accent rounded-lg shadow-lg mb-6">
              <h2 className="text-2xl font-semibold mb-3 text-primary4">Testing & Feedback</h2>
              <div className="text-grey leading-relaxed text-lg">
                <ReactMarkdown  >
                  {project.projectDetails.testingAndFeedback}
                </ReactMarkdown ></div>
          ...
          {project.projectDetails.deploymentOrFinalPrototype && (
            <div id="deployment" className=" p-6  md:p-8 sbg-transparentder border  border-accent rounded-lg shadow-lg mb-6">
              <h2 className="text-2xl font-semibold mb-3 text-primary4">{isDesignProject ? 'Final Prototype' : 'Deployment'}</h2>

              <div className="text-grey leading-relaxed text-lg">
                {isDesignProject ? <><p><strong>live preview:</strong> <a href={project.projectDetails.deploymentOrFinalPrototype.link} target="_blank" > Click me</a></p>
                </> : 
                
                
                <><p><strong>Link:</strong> 
                {project.projectDetails.deploymentOrFinalPrototype.link &&
project.projectDetails.deploymentOrFinalPrototype.isLive !== false ? (
  <a
    href={project.projectDetails.deploymentOrFinalPrototype.link}
    target="_blank"
  ...
  >
     Live Demo
                  ...
                  <p><strong>GitHub:</strong>  {project.projectDetails.deploymentOrFinalPrototype.github &&
project.projectDetails.deploymentOrFinalPrototype.isLivegit !== false? (
  <a
    href={project.projectDetails.deploymentOrFinalPrototype.github}
    target="_blank"
    className="text-primary underline"
  >
    Click Me
  </a>
):project.projectDetails.deploymentOrFinalPrototype.github}</p></>}
              </div>
          ...
          {project.projectDetails.reflectionAndFutureScope && (
            <div id="future" className=" p-6  md:p-8 sbg-transparentder border  border-accent rounded-lg shadow-lg mb-6">
              <h2 className="text-2xl font-semibold mb-3 text-primary4">Future Scope</h2>
              <div className="text-grey leading-relaxed text-lg">
                <ReactMarkdown  >
                  {project.projectDetails.reflectionAndFutureScope}
                </ReactMarkdown ></div>
